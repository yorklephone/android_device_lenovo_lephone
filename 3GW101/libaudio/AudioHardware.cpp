        ...
        if (inputDevice != 0) {
            if (inputDevice & AudioSystem::DEVICE_IN_BLUETOOTH_SCO_HEADSET) {
               ...
               sndDevice = SND_DEVICE_CARKIT;
                ...
                /*if (outputDevices & AudioSystem::DEVICE_OUT_DOCKING_SPK){
                    LOGE("Routing audio to DOCKING\n");
                    sndDevice = SND_DEVICE_DOCKING_SPK;
                } else*/ if(outputDevices & AudioSystem::DEVICE_OUT_SPEAKER) {
                    LOGE("Routing audio to Speakerphone\n");
                    sndDevice = SND_DEVICE_SPEAKER;
                } else if (outputDevices == AudioSystem::DEVICE_OUT_WIRED_HEADPHONE) {
                    LOGE("Routing audio to headphone\n");
                    sndDevice = SND_DEVICE_NO_MIC_HEADSET;
                }
		//lenovo chenyue add for record play begin		
		else if (outputDevices == AudioSystem::DEVICE_OUT_WIRED_HEADSET) {
                    LOGE("Routing audio to Wired Headset\n");
                    sndDevice = SND_DEVICE_HEADSET;	
		//lenovo chenyue add for record play end					
                } else {
