/*
 * Copyright (C) 2008 The Android Open Source Project
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#include <hardware/sensors.h>

#include "nusensors.h"

/*****************************************************************************/
//定义所带的传感器列表
static const struct sensor_t sSensorList[] = {
        { "KXSD9 3-axis Accelerometer",
                "Kionix",
                2050, SENSORS_HANDLE_BASE+ID_A,
                SENSOR_TYPE_ACCELEROMETER, 2.8f, 1.0f/4032.0f, 3.0f, 0, { } },
		//Begin CM3602
		//Copy from liberty
        { "CM3602 Proximity sensor",
                "Capella Microsystems",
                3602, SENSORS_HANDLE_BASE+ID_P,
                SENSOR_TYPE_PROXIMITY, 2000.0f, 1.0f, 6.7f, 0, { } },
        { "CM3602 Light sensor",
                "Capella Microsystems",
                3602, SENSORS_HANDLE_BASE+ID_L,
                SENSOR_TYPE_LIGHT, 2000.0f, 1.0f, 6.7f, 0, { } },
		//End CM3602
};

//声明open_sensors
static int open_sensors(const struct hw_module_t* module, const char* name, struct hw_device_t** device);

//接口：返回设备结构体中设备的个数
static int sensors__get_sensors_list(struct sensors_module_t* module, struct sensor_t const** list){
    *list = sSensorList;
    return ARRAY_SIZE(sSensorList);
}

//接口：对传感器执行打开操作的方式
static struct hw_module_methods_t sensors_module_methods = {
    .open = open_sensors
};

//本HAL模块的相关信息
const struct sensors_module_t HAL_MODULE_INFO_SYM = {
    .common = {
        .tag = HARDWARE_MODULE_TAG,
        .version_major = 1,
        .version_minor = 0,
        .id = SENSORS_HARDWARE_MODULE_ID,
        .name = "WNC Sensors Module",
        .author = "bbs.lephone.cc",
        .methods = &sensors_module_methods,
    },
    .get_sensors_list = sensors__get_sensors_list
};

/*****************************************************************************/

//接口：打开传感器
static int open_sensors(const struct hw_module_t* module, const char* name, struct hw_device_t** device){
	//调用init_nusensors初始化传感器
    return init_nusensors(module, device);
}
